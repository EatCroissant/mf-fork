...
public class ArmourDesign {
    ...
    public static final ArmourDesign SOLID = new ArmourDesign("default", "Basic", 1.0F, 1.0F, 40F, 1.00F)
            .setWeightGroup(light);// Basic Armour
    ...
    private float[] traits = new float[]{1f, 1f, 1f, .6f};
    ...
    public void upd(float[] tr){
        this.traits = tr;
    }

    public void showTraits(){
        System.out.println(Arrays.toString(this.traits));
    }

    public float getTraits(int trait){
        return this.traits[trait];
    }

    public static void show(){
        CLOTH.showTraits();
        FIELDPLATE.showTraits();
    }
    /**
    ...
    public float[] getProtectiveTraits() {
        return new float[]{cuttingModifier, bluntModifier, piercingModifier, .4f};
    }
